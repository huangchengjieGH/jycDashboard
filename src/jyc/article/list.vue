    ...
    <section class="plr-sm">
        ...
        <my-table :data="articleList" :config="tableConfig">
            <div slot="operating" slot-scope="article">
                <router-link :to="'/article/edit?articleId=' + article.item.id">
                    <el-button size="small" type="success">编辑</el-button>
                </router-link>
                <el-button @click="deleteArticleItem(article.item,article.index)" size="small" type="danger">删除
                </el-button>
            </div>
        </my-table>
        ...
        methods: {
            ...
            getArticleList() {
                ...
                Article.prototype.getList(that.search).then(res => {
                    ...
                    that.articleSum = res.data.extra.count || list.length;
                });
            }
        }
    }
